...
class SAC(Trainer):
    ...
    def train_step(self, batch):
        ...
        next_policy_outputs = self.policy(target_input, return_log_prob=True)
        ...
        # for p in self.policy.parameters():
        #     p.requires_grad = True
        policy_outputs = self.policy(concat_input, return_log_prob=True)
        new_actions, policy_mean, policy_log_std, log_pi = policy_outputs[:4]
        q1_new_acts = self.qf1(concat_input, new_actions)
        q2_new_acts = self.qf2(concat_input, new_actions)  ## error
