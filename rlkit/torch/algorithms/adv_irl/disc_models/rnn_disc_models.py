...
class RNNDisc(nn.Module):
    def __init__(
        ...
        self.rnn = None
        if self.rnn_act == 'gru':
            self.rnn = nn.GRU(input_size=hid_dim,
                              hidden_size=hid_dim,
            ...
            self.rnn = nn.LSTM(input_size=hid_dim,
                              hidden_size=hid_dim,
                              num_layers=num_layers,
                              batch_first=True,
                              bias=True,
    ...
    def forward(self, batch): # shape (batch_size, length, dim)
        ...
        output = torch.clamp(output, min=-1.0*self.clamp_magnitude, max=self.clamp_magnitude) #（length, batch_si, dim）
