...
class FakeEnv:
    ...
    def _logprob(self, x: np.ndarray, means: np.ndarray, vars: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        k = x.shape[-1]
        log_prob = -0.5 * (k * np.log(2 * np.pi) + np.sum(np.log(vars), axis=-1) + np.sum((x - means) ** 2 / vars, axis=-1))
        prob = np.sum(np.exp(log_prob), axis=0)
        log_prob = np.log(prob)
        stds = np.mean(np.std(means, axis=0), axis=-1)
        return log_prob, stds

    def step(self, obs: np.ndarray, act: np.ndarray, deterministic: bool = False) -> Tuple[np.array, float, bool, Dict]:
        ...
        means[:, :, 1:] += obs
        stds = np.sqrt(vars)
        samples = means if deterministic else means + np.random.normal(size=means.shape) * stds
        ...
        means = np.concatenate([model_means[:, :1], terminals, model_means[:, 1:]], axis=-1)
        stds = np.concatenate([model_stds[:, :1], np.zeros([batch_size, 1]), model_stds[:, 1:]], axis=-1)
    ...
    def step_tensor(self, obs:torch.Tensor, act: torch.Tensor, deterministic: bool = False) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        assert obs.ndim == act.ndim
        ...
        samples = means if deterministic else means + torch.normal(0, 1, size=means.shape) * stds
