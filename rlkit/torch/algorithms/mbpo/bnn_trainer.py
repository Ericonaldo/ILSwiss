...
class BNNTrainer(Trainer):
    ...
    def __init__(
        ...
        self.num_elites = np.min(self.num_elites, self.bnn.num_nets)
    ...
    def train_step(self, batch: torch.Tensor):
        ...
        self._state: List[List[OrderedDict[str: torch.Tensor]]] = []
        ...
        num_holdout: int = np.min(int(inputs.shape[0] * self.holdout_ratio), self.max_holdout)
        perm: np.ndarray = np.random.permutation(inputs.shape[0])
        ho_inputs = inputs[perm[:num_holdout]]
        ho_inputs: torch.Tensor = np.tile(ho_inputs[None], [self.bnn.num_nets, 1, 1])
        inputs = inputs[perm[num_holdout:]]
        ho_targets = targets[perm[:num_holdout]]
        ho_targets: torch.Tensor = np.tile(ho_targets[None], [self.bnn.num_nets, 1, 1])
        ...
        self.normalizer.set_mean(inp_mean)
        self.normalizer.set_std(inp_std)
        ...
        for epoch in epoch_iter:
            ...
            for batch_num in range(num_batches):
                batch_idx = idxs[:, batch_num * self.batch_size: np.min((batch_num + 1) * self.batch_size, idxs.shape[-1])]
                batch_inp = inputs[batch_idx]
                batch_tar = targets[batch_idx]

                loss = self.compute_loss(batch_inp, batch_tar, add_var_loss=True)
                loss += 0.01 * torch.sum(self.bnn.max_log_var) - 0.01 * torch.sum(self.bnn.min_log_var)
            ...
            train_mse = self.compute_loss(inputs[idxs[:, :self.max_holdout]], targets[idxs[:, :self.max_holdout]], add_var_loss=False)
