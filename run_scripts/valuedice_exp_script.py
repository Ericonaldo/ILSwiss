...
from rlkit.torch.algorithms.valuedice.valuedice import ValueDice
...
def experiment(variant):
    ...
    expert_replay_buffer = EnvReplayBuffer(
        variant["valuedice_params"]["replay_buffer_size"],
        env,
...
if __name__ == "__main__":
    ...
    exp_suffix = "--gp-{}--trajnum-{}".format(
        exp_specs["valuedice_params"]["grad_pen_weight"],
        format(exp_specs["traj_num"]),
    )

    if not exp_specs["valuedice_params"]["no_terminal"]:
